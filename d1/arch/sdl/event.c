...
// --- Touch input (Phase 1) -------------------------------------------------
// g_touch holds the latest polled touch position (bottom-screen pixels).
// g_touch_tapped is a per-frame edge flag: 1 only on the frame the stylus
// first touches down. Touch consumers (OSK, HUD buttons) read these via the
// accessors below so the polling detail stays in event_poll().
static touchPosition g_touch;
static int g_touch_tapped = 0;

/* Returns the latest polled touch position. Valid when touch_tapped() or
 * hidKeysHeld()&KEY_TOUCH is set. */
void get_touch(touchPosition *out) {
	if (out) *out = g_touch;
}

/* Returns 1 on the frame the stylus first touches down, else 0.
 * Call once per consumer per frame; the edge is computed in event_poll(). */
int touch_tapped(void) {
	return g_touch_tapped;
}
//---------------------------------------------------------------------------
...
void event_poll()
{
	...
	hidScanInput();
	// --- Touch input (Phase 1) -------------------------------------------------
	// Poll the touchscreen every frame and track edge state so consumers see a
	// single "tapped" pulse per press (not a continuous held state). The raw
	// position lives in g_touch; touch_tapped() reports a fresh press edge.
	hidTouchRead(&g_touch);
	{
		static u32 old_touch = 0;
		u32 cur_touch = hidKeysHeld() & KEY_TOUCH;
		g_touch_tapped = (cur_touch && !old_touch) ? 1 : 0;
		old_touch = cur_touch;
	}
	//---------------------------------------------------------------------------
...
#ifdef __3DS__
	/* 3DS stereo safety at the authoritative present site. When a window
	 * other than the live game window is front-most (main menu after a demo
	 * ends, in-game MENU, save/options, scores, etc.), force mono on the
	 * GFX_LEFT bank. The demo's per-frame stereo path re-engages
	 * g_stereo_active=1 every frame it plays; once it ends and returns to
	 * the main menu, that flag is still set and gr_flip() (below) becomes a
	 * no-op -- the menu is logically up (tap works) but its top screen is
	 * never drawn (the "can tell the menu is up but can't see it" symptom).
	 * Suspending in DoMenu() only cleared it at entry; re-asserting here, at
