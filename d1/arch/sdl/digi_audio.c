...
/* Re-open the SDL audio device after a temporary NDSP teardown (used
 * around system applets that take the DSP). Guarded: only reopens if
 * audio was ever initialised. */
void digi_audio_reinit()
{
	if (!digi_initialised) return;
	if (SDL_OpenAudio(&WaveSpec, NULL) < 0) {
		Warning("\nError: Couldn't reopen audio: %s\n", SDL_GetError());
		return;
	}
	SDL_PauseAudio(0);
}
