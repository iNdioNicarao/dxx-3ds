...
void ogl_invalidate_textures(void){
	int i;
	if (!gl_initialized)
		return;
	for (i=0;i<OGL_TEXTURE_LIST_SIZE;i++){
		...
		ogl_texture_list[i].wrapstate = -1;
		/* Deliberately do NOT ogl_reset_texture() here. Zeroing the handle
		 * is enough to make ogl_loadbmtexture_f() re-upload, and keeping the
		 * texture's internalformat/format/size intact is required so the
		 * re-upload uses the same pixel format. Resetting to flags=0 would
		 * turn transparent textures (fonts, enemies, weapon fire, reticle)
		 * into GL_RGB, so they re-upload opaque and render solid black. */
	}
}
...
// 3DS automap fix: draw a line between two already-rotated g3s_points using
...
void ogl_start_frame(void){
	...
	// Re-bind picaGL's render target every frame (see pglBindFramebuffer).
	// ROOT CAUSE (v84, re-confirmed v99c): picaGL binds the
	// color/depth buffer only ONCE at init. In citro3d the framebuffer
	// must be re-bound every frame (C3D_FrameDrawOn); after the first
	// gfxScreenSwapBuffers the GPU's framebuffer registers go stale and
	// every later glClear/glDrawArrays writes NOWHERE -> the top screen
	// freezes on the last pre-swap image while the sim still runs (audio
	// fires). That is EXACTLY the briefing->game "frozen on stale
	// frame" symptom. v91 disabled this call with a wrong rationale
	// (it conflated the blank-screen bug -- fixed by v86 force-mono --
	// with this one). Re-enabling fixes the freeze; it does NOT
	// re-introduce the strobe (the strobe was the post-present
	// glClear in gr_flip, fixed separately in v98).
	extern void pglBindFramebuffer(void);
	pglBindFramebuffer();

	// 3D world viewport MUST use the PHYSICAL framebuffer size, not the
	// logical 640x480 canvas. On the 3DS the framebuffer is 400x240; a
	// 640x480 viewport centers the world outside the visible region, so the
	// 3D world never appears (only 2D UI shows). Match ogl_end_frame (2D),
	// which already uses the physical sc_w/sc_h.
	OGL_VIEWPORT(grd_curcanv->cv_bitmap.bm_x,grd_curcanv->cv_bitmap.bm_y,grd_curscreen->sc_w,grd_curscreen->sc_h);
	...
	// v98: also clear COLOR at frame start (not after present). Previously
	// gr_flip() cleared color AFTER presenting, which blacked colorBuffer
	// before the per-frame SECOND gr_flip() (event_process) presented it ->
	// the 30 Hz scene<->black strobe (cblum pulsing 0<->153 at 2-frame period,
	// confirmed via lum_trace on-device). render_frame() redraws the whole
	// world each frame, so starting clean here is correct and removes the
	// spurious black present.
...
void gr_flip(void)
{
	...
	// v98: REMOVED glClear(GL_COLOR_BUFFER_BIT) here. It blacked colorBuffer
	// AFTER the present, so the per-frame second gr_flip() (event_process,
	// event.c:381) presented a black buffer -> 30 Hz scene<->black strobe.
	// Color is now cleared at ogl_start_frame (frame start) instead.
}
