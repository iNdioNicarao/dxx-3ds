...
{
#ifdef __3DS__
	// When stereo 3D is active, the eye pair is already presented inside
	// game_render_frame_mono (two pglSwapBuffers: LEFT, then RIGHT+present).
	// Skip this present to avoid a third/duplicate swap.
...
#ifdef __3DS__
	pglInit();
	// NOTE: pglSetStereo is set in LOCKSTEP with gfxSet3D inside
	// game_render_frame_mono (only when the 3D slider is up). Do NOT set it
	// here unconditionally: pglSetStereo(true) with gfxSet3D(false) makes
	// picaGL enter its stereo branch against a 2D top framebuffer, which
	// hangs at level load (broken power-off). Default (stereo off) = proven
	// mono present path.
#else
#ifndef RPI
