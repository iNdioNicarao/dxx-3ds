...
#ifdef __3DS__
	// On 3DS there is no physical keyboard for text entry, so auto-default
	// the pilot name instead of prompting. The player can still rename later
	// if a bottom-screen keyboard is added.
	if (text[0] == 0) {
		char default_name[] = "player";
		int n = 1;
		snprintf(text, sizeof(text), "%s", default_name);
		// Ensure the default doesn't collide with an existing pilot file.
		for (;;) {
...
int RegisterPlayer()
{
...
#ifdef __3DS__
	// The 3DS has no physical keyboard, and the pilot listbox relies on
	// KEY_ENTER / arrow key commands that our synthetic HID key pulses
	// cannot reliably deliver (zero-scancode events don't map to KEY_ENTER).
	// Bypass the listbox entirely: load the default "player" pilot if it
	// already exists on the card, otherwise create it. This is the same
	// auto-default behaviour used inside MakeNewPlayerFile().
	if (Players[Player_num].callsign[0] == 0)
		strncpy(Players[Player_num].callsign, "player", CALLSIGN_LEN);

	{
		...
		snprintf(pname, sizeof(pname),
		         GameArg.SysUsePlayersDir ? "Players/%s.plr" : "%s.plr",
		         Players[Player_num].callsign);
		if (PHYSFSX_exists(pname, 0) && read_player_file() == EZERO)
			return 0;
	}
	MakeNewPlayerFile(0);
	return 0;
