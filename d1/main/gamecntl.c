		...
		case EVENT_KEY_COMMAND:
			...
			switch (key)
			{
				...
				case KEY_PAUSE:
					...
					return 1;
#ifdef __3DS__
				case -1: /* synthetic idle (forwarded by newmenu listbox etc.) */
#endif
				default:
					break;
			}
...
int ReadControls(d_event *event)
{
	...
	if (event->type == EVENT_KEY_COMMAND)
	{
		...
		{
#ifndef __3DS__
			/* Typed-cheat detection is disabled on 3DS: the handheld has
			 * no keyboard, and synthetic button ASCII was leaking into the
			 * cheat buffer (auto-firing "Cheater"). Cheats are reachable
			 * only through the main-menu Cheats entry / do_cheat_menu(). */
