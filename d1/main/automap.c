...
// 3DS automap fix: draw lines via the GL modelview (p3_vec) path so they
// align with the marker sphere, instead of the broken gr_line(p3_sx,p3_sy)
// 2D path which does not render on the 3DS. ogl_draw_line_vec is defined in
// arch/ogl/ogl.c (mirrors g3_draw_sphere's push/translate technique).
extern void ogl_draw_line_vec(g3s_point *p0, g3s_point *p1);
# define automap_draw_line ogl_draw_line_vec
...
void do_automap( int key_code )
{
	...
	/* 3DS: force stereo OFF for the duration of the automap overlay so its
	 * present lands on the visible (mono) bank, not the hidden RIGHT bank a
	 * stereo session leaves selected. Same fix as the menu/banner overlays. */
	stereo_suspend();
...
void draw_all_edges(automap *am)	
{
	...
	for (i=0; i<=am->highest_edge_index; i++ )	{
		...
		// 3DS fix: rotate the edge endpoints fresh under the *current*
		// (automap) view matrix. rotate_list() caches by framecount, but the
		// main renderer already stamped Segment_points[] with this framecount
		// under the GAMEPLAY view, so it would skip re-rotating them here and
		// every edge would keep stale gameplay-screen coordinates -> invisible
		// automap. g3_rotate_point always recomputes p3_vec/p3_codes (and clears
		// PF_PROJECTED so g3_draw_line projects it fresh). g3_draw_line projects.
		g3_rotate_point(&Segment_points[e->verts[0]], &Vertices[e->verts[0]]);
		g3_rotate_point(&Segment_points[e->verts[1]], &Vertices[e->verts[1]]);
		cc.uand = Segment_points[e->verts[0]].p3_codes & Segment_points[e->verts[1]].p3_codes;
		cc.uor  = Segment_points[e->verts[0]].p3_codes | Segment_points[e->verts[1]].p3_codes;
			...
			if ( nfacing && nnfacing )	{
				...
				if ( nfacing == 0 )	{
					if ( e->flags & EF_NO_FADE )
						...
						ogl_draw_line_vec( &Segment_points[e->verts[0]], &Segment_points[e->verts[1]] );
				} 	else {
					am->drawingListBright[nbright++] = e-am->edges;
				}
			}
