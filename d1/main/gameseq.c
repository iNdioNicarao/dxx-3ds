...
{
#ifdef __3DS__
	/* Reset the stereo/present state at the very start of a fresh game.
	 * After a demo (or any prior stereo session) ends, the last on-screen
	 * frame left g_stereo_active=1 and the display bank on GFX_RIGHT. If we
	 * don't clear that here, LoadLevel's "Prepare for Descent" banner flips
	 * to the hidden RIGHT bank while game_render_frame_mono re-engages stereo
	 * on the next frame -> the banner alternates over the game view a few
	 * times a second. (Repro: watch a demo, then New Game.) This mirrors the
	 * stereo_resume() that game_leave_menus() already does on the
	 * menu->resume path, which is why MENU then RESUME clears the same
	 * symptom. stereo_resume() is idempotent: it just clears the flags and
	 * re-selects GFX_LEFT; the per-frame slider logic re-engages 3D cleanly
	 * on the first gameplay frame if the slider is still up. */
	stereo_resume();
#endif
