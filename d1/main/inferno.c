...
int main(int argc, char *argv[])
{
    ...
    mcuHwcInit();
	hidInit();
	/* IMPORTANT: allocate the bottom as a TRUE RGB565 framebuffer.
	 * gfxInitDefault() allocates BOTH screens as BGR8 (3 bytes/px -> 720
	 * bytes/row). Calling gfxSetScreenFormat(GFX_BOTTOM, GSP_RGB565_OES)
	 * AFTER that only changes the *format tag*; libctru does NOT reallocate
	 * the buffer (it keeps the larger BGR8 region), so the bottom is still
	 ...
	 * Double-buffering the bottom while we only flush (never swap) made the
	 * display alternate our buffer with a stale/empty one -> the strobing.
	 * The top stays double-buffered (picaGL swaps it). */
...
#define DXX_MISSING_HOGFILE_ERROR_TEXT	\
	...
	setjmp(LeaveEvents);
#ifdef __3DS__
	// Check the app's run state every frame, OUTSIDE the event-input path,
	// so power-off / home-menu requests are always honoured even when a
	// blocking menu or the power-off screen is up. Also bail the instant
	// APT requests power-off (d1x_powering_off, set by the APT ONSUSPEND
	// hook): without this, a power press during e.g. "New Game" would let
	// the new level load + render on a GPU the OS is tearing down, which
	// is a Data Abort on the GPU/LCD registers.
	while (window_get_front() && aptMainLoop() && !d1x_powering_off)
#else
	while (window_get_front())
#endif
	{
	...
	// returned false) vs. hanging inside the loop.
	// Power-off / APT-exit path: aptMainLoop() returned false because the OS
	// is tearing the app down. Stop the audio DSP thread FIRST (while the DSP
	// service is still alive) so ndspThreadRun can't dereference freed memory
	// during OS teardown -- that was the ARM11 crash-at-shutdown (DFAR=0x1).
	// digi_close() -> SDL_CloseAudio() -> ndspExit(). This path runs only on
	// real exit (aptMainLoop false), never on HOME/sleep (which keep looping),
	...
	if (!GameArg.SndNoSound)
		digi_close();
	pglSetPoweredOff();
	return 0;   /* libctru appExit -> gfxExit on live display = clean exit */
#else
