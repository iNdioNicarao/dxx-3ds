...
#define DXX_MISSING_HOGFILE_ERROR_TEXT	\
	...
	// blocking menu or the power-off screen is up. (Fixes the "tap Power Off
	// and hang" regression that occurred when this check lived inside event_poll.)
	while (window_get_front() && aptMainLoop())
#else
	while (window_get_front())
#endif
	{
