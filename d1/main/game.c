...
#include "game.h"
#ifdef __3DS__
#include "texmerge.h"      /* texmerge_flush() — rebuild wall cache after menu/PCX render */
#include <GL/picaGL.h>    /* pglReacquire() — full GPU/texture recovery after menu/lid */
#endif
#include "key.h"
#include "object.h"
#include "physics.h"
...
// 3DS: on-screen control reference for the 3DS button mapping.
void show_controls_3ds()
{
	int nitems = 0;
	newmenu_item *m;

	MALLOC(m, newmenu_item, 40);
	if (!m)
		return;

	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "3DS CONTROLS";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "FLIGHT";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "Circle Pad ..... Turn / Pitch";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "C-Stick ........ Slide (up/down/left/right)";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "X .............. Accelerate";
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "B .............. Reverse";
	...
	m[nitems].type = NM_TYPE_TEXT; m[nitems++].text = "A .............. Select      B ....... Back";
...
window *game_setup(void)
{
	...
	Game_suspended = 0;
	time_paused = 0;	// re-arm sim clock; never inherited from a prior aborted game
	reset_time();
...
int game_handler(window *wind, d_event *event, void *data)
{
	...
	/* 3DS: if a demo was selected to play, demo_menu_handler set this flag
	 * instead of calling game_leave_menus() inline (which would re-enter
	 * listbox_handler with a dangling event). Run it here, from the
	 * game-loop context, after the selector handler unwound, so the
	 * window-close that shuts the demo list fires a clean EVENT_WINDOW_CLOSED. */
	if (d1x_defer_leave_menus) {
		d1x_defer_leave_menus = 0;
		game_leave_menus();
	}

	/* 3DS: Quit from the in-game MENU overlay. Closing Game_wind from the
	 * menu handler itself would fire EVENT_WINDOW_CLOSED (longjmp) mid
	 * modal-dispatch, so main_menu_handler defers it here — same pattern
	 * as d1x_defer_leave_menus above. window_close(Game_wind) runs the
	 * standard abort flow (EVENT_WINDOW_CLOSE cleanup, GM_GAME_OVER,
	 * bottom buttons cleared) and returns to the main menu.
	 *
	 * Before closing Game_wind, close any overlay windows sitting above it
	 * (the in-game DoMenu modal that opened the menu). If left in place,
	 * that modal becomes window_get_front() after Game_wind closes, so the
	 * next game's freshly-created Game_wind is never front-most — and
	 * flight controls are gated on (window_get_front() == Game_wind), so
	 * movement/fire would be dead on the new game (a regression introduced
	 * when Quit started actually closing the game window). */
	if (d1x_defer_quit_game) {
		d1x_defer_quit_game = 0;
		con_printf(CON_URGENT, "[DBG] defer_quit: time_paused before=%d\n", time_paused);
		window *wind;
		while ((wind = window_get_front()) && (wind != Game_wind))
			window_close(wind);
	...
	switch (event->type)
	{
		case EVENT_WINDOW_ACTIVATED:
			...
			reset_cockpit();
#ifdef __3DS__
			/* Fresh draw of the persistent in-game MENU + SAVE buttons. */
			bottom_menu_reset();
			bottom_save_reset();
			bottom_rec_reset();
#endif
		...
		case EVENT_IDLE:
#ifdef __3DS__
		/* The 3DS has no ESC key, so the bottom-screen MENU button opens
		 * the MAIN MENU as an overlay on top of the running game
		 * (Game_wind is kept alive). The main menu gains a 3DS-only
		 * "Resume Game" text entry (selectable like any other menu item)
		 * that calls game_leave_menus() to return to where the player was
		 * - this mirrors how a PC port would expose resume, and the 3DS
		 * already navigates menus via touch/d-pad. The only 3DS-specific
		 * UI additions are things with no button equivalent: the MENU
		 * button (no ESC key) and the SAVE/REC buttons (no F2/F5 keys).
		 * The SAVE button saves in-game via state_save_all(0) (PC's F2);
		 * both SAVE and REC are only valid while a level is loaded
		 * (Current_level_num > 0), never from the main menu.
		 *
		 * During demo PLAYBACK the bottom buttons must be hidden: SAVE/REC
		 * are meaningless while a demo plays and MENU would open the main
		 * menu over a running demo. Demo playback leaves Current_level_num
		 * > 0 (level still loaded), so the in-game gate alone wouldn't hide
		 * them -- gate explicitly on Newdemo_state. On the playback->stop
		 * edge we blank + reset once so any button drawn before playback
		 * is erased; while playing we skip the polls so they aren't
		 * (re)drawn. Returning to normal play redraws them (reset put each
		 * button back to its undrawn state). */
		static int bs_was_playback = 0;
		if (Newdemo_state == ND_STATE_PLAYBACK) {
			if (!bs_was_playback) {
				bottom_menu_reset();
				bottom_save_reset();
			...
			bs_was_playback = 0;
		if (bottom_menu_tapped()) {
			{
			}
			DoMenu();
			{
			}
			/* Blank the in-game buttons while the main menu overlay is up.
			 * The game redraws them on resume via game_leave_menus(). */
			bottom_menu_reset();
			bottom_save_reset();
			bottom_rec_reset();
			bottom_clear(0);
		...
		if (Current_level_num > 0 && bottom_save_tapped()) {
			...
			if (bottom_rec_tapped()) {
				 ...
				 * otherwise unreachable. Toggle it: start when idle,
				 * stop (prompting for a save name) when already recording. */
				if (Newdemo_state == ND_STATE_RECORDING)
		...
		case EVENT_WINDOW_DRAW:
			// 3DS: when a modal overlay (MENU/Save/pause) is open in front
			// of Game_wind, do not advance the simulation -- freezing it
			// keeps the level/briefing from loading or running behind the
			// menu and avoids the game/overlay draw racing each other. The
			// frozen frame is still drawn below so event.c's single flip
			// presents the overlay on top of a static backdrop. Without
			// this, selecting e.g. New Game drove StartNewLevelSub (briefing
			// + level load) while the menu was still up, alternating with
			// the menu every frame.
			if (!time_paused && (Game_wind == window_get_front()))
			{
				calc_frame_time();
				GameProcessFrame();
			}
...
int d1x_defer_leave_menus = 0;
int d1x_defer_quit_game = 0;
...
void game_leave_menus(void)
{
	...
	/* 3DS: leaving a menu (demo select, pause/resume, pilot, options, …) and
	 * resuming gameplay must not carry the menu's stale picaGL/GPU state into
	 * the next heavy render. A menu renders a PCX background and leaves a
	 * texture bound; lid-resume may also have left picaGL partially recovered.
	 * When gameplay then loads a level (e.g. starting demo playback) it can
	 * data-abort on the GPU/LCD registers. Do a full GPU/texture recovery and
	 * rebuild the wall (texmerge) cache so the first gameplay frame is clean.
	 * Idempotent; safe to call on every menu-exit. */
#ifdef __3DS__
	pglReacquire();   /* full picaGL GPU recovery (shader/buffers + texture reset) */
