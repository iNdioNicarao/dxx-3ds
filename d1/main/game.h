...
void do_cheat_menu(void);   // in-game "wimp" cheat menu (gamecntl.c)
...
/* 3DS stereo: suspend the stereo present while a menu/pause overlay is up
 * over the live game (the overlay's mono present contends with the eye-pair
 * path and wedges the render thread), resume lets the per-frame slider logic
 * re-engage. See gamerend.c. */
void stereo_suspend(void);
void stereo_resume(void);
/* 3DS: deferred game_leave_menus() trigger. demo_menu_handler sets this when
 * a demo is selected to play, because calling game_leave_menus() directly
 * and game_handler() runs game_leave_menus() from the game-loop context
 * (after the selector handler has fully unwound), so the window-close that
 * shuts the demo list fires a clean EVENT_WINDOW_CLOSED. */
extern int d1x_defer_leave_menus;

/* 3DS: Quit (abort) from the in-game MENU overlay is deferred to the game
 * window's handler for the same reason as d1x_defer_leave_menus — the menu
 * handler must not close Game_wind mid modal-dispatch. Set by
 * main_menu_handler's MENU_QUIT; honoured at the top of game_handler(). */
extern int d1x_defer_quit_game;
