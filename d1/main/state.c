...
int state_get_savegame_filename(char * fname, char * dsc, char * caption, int blind_save)
{
	...
	for (i=0;i<NUM_SAVES; i++ )	{
		...
		if (dsc != NULL) {
#ifdef __3DS__
			/* 3DS has no physical keyboard, so newmenu's built-in text input
			 * (NM_TYPE_INPUT_MENU) is a dead end. Use a plain menu item; the
			 * save name is collected via swkbd after the slot is picked
			 * (see below). */
	...
	if (choice > 0) {
#ifdef __3DS__
		if (dsc != NULL && strcmp(desc[choice-1], TXT_EMPTY) == 0) {
			/* 3DS: collect the save name with swkbd (newmenu's built-in
			 * input box has no keyboard here). Empty/cancelled -> abort. */
			char nm[DESC_LENGTH+1];
			nm[0] = 0;
			osk_modal_loop(nm, sizeof(nm));
			if (nm[0] == 0) {
				for (i=0; i<NUM_SAVES; i++)
					if (sc_bmp[i]) gr_free_bitmap(sc_bmp[i]);
				return 0;
			}
			strncpy(desc[choice-1], nm, DESC_LENGTH-1);
			desc[choice-1][DESC_LENGTH-1] = 0;
		}
#endif
...
// 3DS/picaGL: glReadPixels is a no-op stub and an offscreen render_frame() into
// a tiny canvas corrupts picaGL's live wall-texture cache (walls go black on
// return to game). Skip the capture entirely on 3DS and write a blank
// thumbnail (the load menu simply shows no preview). This is the SAME guard as
// state_save_all_sub() — both paths must skip the offscreen render.
#ifndef __3DS__
...
#ifdef __3DS__
	/* 3DS/picaGL: returning from the in-game SAVE name entry must re-enter
	 * the live gameplay render, not just replay picaGL cached state.
	 * game_leave_menus() does the full recovery (pglReacquire rebinds the
	 * GX queue + shader/texenv, texmerge_flush re-pages the wall textures)
	 * and closes any stray menu window above Game_wind. A bare
	 * pglReacquire()+texmerge_flush() left the walls black because the
	 * gameplay render wasn't re-entered. */
	game_leave_menus();
#endif
...
int state_quick_save(void)
{
	...
	snprintf(filename, PATH_MAX, "%s.qusg1", Players[Player_num].callsign);

	desc[0] = '\0';
#ifdef HAVE_STRING_H
	strncat(desc, "Quick Save", DESC_LENGTH);
#else
	strcpy(desc, "Quick Save");
#endif
...
// 3DS: name-free manual load of the quick-save written by state_quick_save().
// Reads the fixed .qusg1 file so no on-screen keyboard / menu is needed.
int state_quick_load(void)
{
	...
	stop_time();
	if (!state_restore_all_sub(filename)) {
		start_time();
		return 0;
	}
...
// Save the current screen shot...
// 3DS/picaGL: glReadPixels is a no-op stub and an offscreen render_frame()
// into a tiny canvas corrupts picaGL's live wall-texture cache (walls go
// black on return to game). Skip the capture entirely and write a blank
// thumbnail. The load menu simply shows no preview on 3DS.
#ifndef __3DS__
	cnv = gr_create_canvas( THUMBNAIL_W, THUMBNAIL_H );
	if ( cnv )
	{
