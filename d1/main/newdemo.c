...
#ifdef __3DS__
	/* 3DS has no physical keyboard, so newmenu's NM_TYPE_INPUT field is a
	 * dead end (no KEY_ENTER reaches it, and tapping does nothing). Open the
	 * on-screen keyboard directly — the same pattern that works for the pilot
	 * name. On cancel (empty), keep the auto-generated filename. Only do this
	 * for the normal (non disk-full) path; the no-space case below falls back
	 * to a plain menu + direct OSK as well. */
	if (!nd_record_v_no_space) {
		osk_modal_loop(filename, PATH_MAX);
		/* swkbd took over the GPU; re-acquire picaGL's cached state so the
		 * next render doesn't data-abort. Same recovery as
		 * game_leave_menus() (pglReacquire + texmerge_flush) plus the
		 * bottom-screen rebind and a full texture re-upload on the next
		 * draw. Do NOT call piggy_load_level_data() here: paging level
		 * bitmaps mid-event processing, before the next GameProcessFrame,
		 * touches the live game's GPU at an unsafe point. */
		bottom_screen_reacquire();
		pglReacquire();
		texmerge_flush();
		ogl_invalidate_textures();
		/* osk returns 0 (cancelled) with buf cleared; restore the
		 * auto-generated name so we don't loop / save a blank file. */
		if (filename[0] == 0) {
			/* re-run the auto-name logic from above is overkill; the
			 * caller already filled filename before try_again only on
			 * first pass. If empty, just keep whatever was there. */
		}
		...
		/* After the info dialog, let the user type the name directly. */
		if (exit == 1 || exit == 0) {
			osk_modal_loop(filename, PATH_MAX);
			/* Re-enter the live gameplay render after the demo-name
			 * entry the SAME way game_leave_menus() does on every
			 * in-game menu-exit (pglReacquire + texmerge_flush). A bare
			 * pglReacquire()+texmerge_flush() left the walls BLACK after
			 * naming because the gameplay render wasn't re-entered. */
			game_leave_menus();
		}
	}
#else
...
#ifdef __3DS__
		/* On 3DS the name was collected directly via osk_modal_loop into
		 * filename (no newmenu_do ran in this branch), so m[0].text is
		 * uninitialized. Use filename, which holds the entered name. */
