...
#include "gamefont.h"
#ifdef __3DS__
#include "bottom_screen.h"   /* tappable "Reset Scores" key (PC: Ctrl+R) */
#endif
#include "mouse.h"
#include "joy.h"
#include "timer.h"
...
#ifdef __3DS__
		{
			/* The PC build resets high scores with the Ctrl+R combo, which
			 * the 3DS has no key for (the screen's "Press CTRL+R to reset"
			 * prompt is unreachable). While this window is on the top
			 * screen, draw a tappable "Reset Scores" key on the bottom
			 * screen; on a fresh tap feed the identical Ctrl+R command back
			 * into this handler's EVENT_KEY_COMMAND path so the SAME
			 * confirmation box + file deletion + scores reload runs.
			 * Enabled only when no score row is highlighted — the PC combo
			 * only works then, and the prompt is only shown then. */
			int enable = (menu->citem < 0);
			if (bottom_scores_rst_tapped(enable)) {
				/* Synthesize the Ctrl+R key command. d_event_keycommand is
				 * private to key.c, so use a compatible local struct (stable
				 * ABI: { event_type type; int keycode; }), as the pilot/demo
				 * delete buttons do. */
				struct { event_type type; int keycode; } rst_evt;
				rst_evt.type = EVENT_KEY_COMMAND;
				rst_evt.keycode = KEY_CTRLED + KEY_R;
				scores_handler(wind, (d_event *)&rst_evt, menu);
			}
...
#ifdef __3DS__
			/* Clear the bottom-screen Reset Scores key so it doesn't linger
			 * after the scores window closes. */
			bottom_clear(0);
			bottom_screen_present();
#endif
			d_free(menu);
...
#ifdef __3DS__
	/* Fresh draw of the bottom-screen Reset Scores key when the scores
	 * window opens, and clear the bottom to a clean panel (otherwise the
	 * previous screen's content bleeds through). */
	bottom_scores_rst_reset();
	bottom_clear(0);
	bottom_screen_present();
#endif
