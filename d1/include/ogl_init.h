...
/* Invalidate every GL texture handle so all textures re-upload on their
 * next draw. Needed after a system applet (swkbd, HOME) has taken the GPU
 * and returned: picaGL texture state is not guaranteed to survive the
 * applet, and bitmaps with a live handle are otherwise never re-uploaded,
 * which leaves walls rendering black. Cheaper than the video-mode switch
 * smash: leaves the shared sphere/circle/disk vertex arrays intact. */
